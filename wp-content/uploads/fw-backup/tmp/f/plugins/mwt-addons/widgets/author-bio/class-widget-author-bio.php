...
if ( ( defined( 'FW' ) ) && ! ( class_exists( 'MWT_Widget_Author_Bio' ) ) ) :

	class MWT_Widget_Author_Bio extends WP_Widget {
		...
		function __construct() {
			...
			$widget_ops = array (
				'classname'   => 'widget_author_bio',
				'description' => esc_html__( 'Add image with text', 'mwt' ),
			);
			...
			parent::__construct( false, esc_html__( 'Theme - Author Bio', 'mwt' ), $widget_ops );
			...
			$this->options = array (
				...
				'image' => array (
					'type'        => 'upload',
					'value'       => '',
					'label'       => esc_html__( 'Author Image', 'mwt' ),
					'images_only' => true,
				),
				'name'   => array (
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Author Name', 'mwt' ),
				),
				'info'   => array (
					'type'  => 'textarea',
					'value' => '',
					'label' => esc_html__( 'Biographical Info', 'mwt' ),
				),
				$social_icons,
			);
			$this->prefix  = 'widget_author_bio';
		}
		...
		function widget( $args, $instance ) {
			...
			$filepath = MWT_WIDGETS_PLUGIN_PATH . '/widgets/author-bio/views/widget.php';

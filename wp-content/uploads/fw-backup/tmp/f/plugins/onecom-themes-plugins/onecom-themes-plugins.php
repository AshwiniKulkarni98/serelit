...
if ( ! defined('ONECOM_WP_VERSION') ) {
	define('ONECOM_WP_VERSION', '4.4.4');
}
...
// WP Rocket CP purchase URL with utm tracking params.
if ( ! defined('OC_WPR_BUY_URL') ) {
	$domain        = $_SERVER['ONECOM_DOMAIN_NAME'] ?? '';
	$oc_utm_medium = oc_utm_medium() ?? 'wp_rocket';
	define('OC_WPR_BUY_URL', sprintf("https://one.com/admin/wprocket-prepare-buy.do?directToDomainAfterPurchase=%s&amp;domain=%s&amp;utm_source=onecom_wp_plugin&amp;utm_medium=$oc_utm_medium", OC_HTTP_HOST, $domain));
}
...
if ( ! function_exists('onecom_wp_load_textdomain') ) {
	function onecom_wp_load_textdomain() {
		...
		if ( $current_locale === 'fi' ) {
			load_textdomain(OC_PLUGIN_DOMAIN, dirname(__FILE__) . '/languages/onecom-wp-fi_FI.mo');
		} else if ( $current_locale === 'nb_NO' ) {
			load_textdomain(OC_PLUGIN_DOMAIN, dirname(__FILE__) . '/languages/onecom-wp-no_NO.mo');
			...
			load_textdomain(OC_PLUGIN_DOMAIN, dirname(__FILE__) . '/languages/onecom-wp-en_GB.mo');
		}
	}
}
...
if ( ! function_exists('one_core_admin') ) {
	function one_core_admin() {
		...
		add_submenu_page(
			$parent_slug = '', // Adding empty to hide from submenu.

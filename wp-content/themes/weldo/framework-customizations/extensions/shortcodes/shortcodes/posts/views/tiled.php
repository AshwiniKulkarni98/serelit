	...
	while ( $posts->have_posts() ) : $posts->the_post();
		...
		if ( get_template() === 'weldo') {
			if ( $i == 1 ) {
				...
				include get_template_directory() . '/framework-customizations/extensions/shortcodes/shortcodes/posts/views/item-tiled2.php';
				echo '</div>';
			} elseif ( $count >= 3 ) {
				if ( $i == 2 ) {
					echo '<div class="col-lg-7">';
				}
				include get_template_directory() . '/framework-customizations/extensions/shortcodes/shortcodes/posts/views/item-tiled.php';
				if ( $i == 3 ) {
					echo '</div>';
				}
			} elseif ( $count < 3 ) {

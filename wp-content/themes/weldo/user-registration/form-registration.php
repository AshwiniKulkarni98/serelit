...
?>
	<div class='user-registration ur-frontend-form <?php echo esc_attr( $template_class . ' ' . $custom_class ); ?>' id='user-registration-form-<?php echo absint( $form_id ); ?>'>
		<form method='post' class='register'
		      data-enable-strength-password="<?php echo esc_attr( $enable_strong_password ); ?>" data-minimum-password-strength="<?php echo esc_attr( $minimum_password_strength ); ?>" <?php echo apply_filters( 'user_registration_form_params', '' ); ?>>
			...
			if ( $is_field_exists ) {
				...
				?>
				<div class="ur-button-container form-row submit-row <?php echo esc_html( implode( ' ', $btn_container_class ) ); ?>" >
					<?php
					...
					<button type="submit" class="btn btn-maincolor button ur-submit-button <?php echo esc_html( implode( ' ', $submit_btn_class ) ); ?>">
						<span></span>
						<?php
					...
					<a class="btn btn-outline-maincolor2 register-btn" data-toggle="modal" data-dismiss="modal" href="#login-form"><?php echo esc_html__( 'Log In','weldo' ) ?></a>
			...
			if ( count( $form_data_array ) == 0 ) {
				?>
				<h2><?php echo esc_html__( 'Form not found, form id :' . $form_id, 'weldo' ); ?></h2>
				<?php
			}
			?>
			...
			<input type="hidden" name="ur-redirect-url" value="<?php echo esc_url( $redirect_url ); ?>"/>

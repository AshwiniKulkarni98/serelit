...
class OnecomCheckPlugins extends OnecomHealthMonitor {
	...
	public function fix_dis_plugin(): array {
		...
		// If no active discouraged plugins remain, mark as resolved
		if ( empty( $dis_plugins ) ) {
			return $this->format_result(
				$this->flag_resolved,
				$this->text['dis_plugin'][ $this->fix_confirmation ],
				$this->text['dis_plugin'][ $this->status_desc ][ $this->status_resolved ]
			);
		}

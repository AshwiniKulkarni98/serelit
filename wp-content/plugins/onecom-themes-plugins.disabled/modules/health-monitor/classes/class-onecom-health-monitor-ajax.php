...
class OnecomHealthMonitorAjax extends OnecomHealthMonitor {
	...
	public function init() {
		...
		add_action( 'wp_ajax_ocsh_get_last_scan_time', array( $this, 'get_last_scan_time' ) );
		$this->add_check_callbacks();
		$fixes->init();
		$this->add_undo_callbacks();
	}

	public function get_last_scan_time(): void {
		$scan_result = get_site_transient( 'ocsh_site_scan_result' );
		$time        = is_array( $scan_result ) && isset( $scan_result['time'] ) ? $scan_result['time'] : false;

		if ( $time && is_numeric( $time ) && function_exists( 'wp_date' ) ) {
			$frmt = 'l ' . get_site_option( 'date_format' ) . ' ' . get_site_option( 'time_format' );
			$tz   = get_site_option( 'timezone_string' );
			$tz   = ! empty( $tz ) ? $tz : 'UTC';
			wp_send_json_success( wp_date( $frmt, $time, new DateTimeZone( $tz ) ) );
		} else {
			wp_send_json_success( __( 'No scan available', 'onecom-wp' ) );
		}
	}
	...
	public function send_json( array $result, string $check = '' ) {
		...
		$data[] = $this->prepare_single_check_data( $check, $result );
...
//		}
		...
		if ( ! ( defined( 'REST_REQUEST' ) || defined( 'DOING_CRON' ) ) ) {
			wp_send_json_success( $data ); // Instead of raw HTML
		}
	...
	public function pingbacks() {
		...
		$result['fix']      = true;
		$result['undo']     = true;
		$result['fix_text'] = __('Turn off pingbacks', 'onecom-wp' );
	...
	public function ocsh_mark_resolved() {
		...
		if ( $result ) {
			wp_send_json( $this->format_result( $this->flag_resolved, __( 'The item was ignored and moved to “Ignored”.', 'onecom-wp' ) ) );
		} else {
			wp_send_json( $this->format_result( $this->flag_open, __( 'Failed to ignore the item.', 'onecom-wp' ) ) );
		}
	}
	...
	public function unignore(): void {
		...
		$result = update_option( 'oc_marked_resolved', $marked_as_resolved, 'no' );
		if ( $result ) {
			wp_send_json( $this->format_result( $this->flag_resolved, __( 'The item was unignored and will be part of future scans.', 'onecom-wp' ) ) );
		} else {
			wp_send_json( $this->format_result( $this->flag_open, __( 'Couldn’t unignore the item.', 'onecom-wp' ) ) );
		}
	}

...
class OnecomFixes extends OnecomHealthMonitor {
	...
	public function fix_usernames() {
		...
		$this->remove_from_ignore( 'usernames' );
		wp_send_json( $result );
	}
	...
	private function remove_from_ignore( string $check_name = '' ): void {

		if ( $check_name === '' ) {
			$check_name = strip_tags( $_POST['action'] );
			$check_name = str_replace( array( 'ocsh_fix_', 'ocsh_fix_check_', 'ocsh_change_', 'check_' ), '', $check_name );
		}
		$check              = $check_name;

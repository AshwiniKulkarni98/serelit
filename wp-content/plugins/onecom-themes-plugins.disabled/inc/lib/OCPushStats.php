...
if ( ! class_exists( 'OCPushStats' ) ) {
	...
	final class OCPushStats {
		...
		public static function stats_base_parametres() {
			...
			$param['fqdn']                   = $_SERVER[self::HTTP_HOST] ?? '';
			$param['brand']                  = $_SERVER[self::ONECOM_BRAND] ?? '';
			$user                            = self::get_users();
			$param                           = array();
			$param[ self::DOMAIN ]           = self::get_domain();
			$param[ self::SUB_DOMAIN ]       = self::get_subdomain();
			$param[ self::WP_USER ]          = $user['name'] ?? null;
			$param[ self::WP_ROLE ]          = $user['role'];
			$param['ip']                     = self::onecom_get_client_ip_env();
			$param['php_version']            = PHP_VERSION;
			$param[ self::WP_LANG ]          = $get_version[self::WP_LANG] ?? null;
			$param[ self::WP_VERSION ]       = $get_version[ self::WP_VERSION ] ?? null;
			$param[ self::PLUGIN_VERSION ]   = $get_version[ self::PLUGIN_VERSION ] ?? null;
			$param[ self::USER_AGENT ]       = $_SERVER[ self::SERVER_AGENT ] ?? null;
			$param[ self::PACKAGE_FEATURES ] = isset( $get_features['data'] ) ? json_encode( $get_features['data'] ) : null;
			$param[ self::HOSTING_PACKAGE ]  = $get_features['hosting_package'] ?? null;

			if(isset( $_SERVER[ self::ONE_COM_CLUSTER_ID ] ) && !empty( $_SERVER[ self::ONE_COM_CLUSTER_ID ] )){
				$param['cluster'] = $_SERVER[ self::ONE_COM_CLUSTER_ID ];
				$param['webroot'] = self::oc_get_web_root();
			}
			return $param;
		}
		...
		public static function push_vul_monitor_stats(
			...
			if ( ! empty( $additional_info ) ) {
                ...
                foreach ( $additional_info as $key => $value ) {
                    $dynamic_params[ $key ] = json_encode( $value );
                }
			}
			$payload = json_encode( array_merge( $base_params, $dynamic_params ) );
			return self::curl_request( $payload );

...
class Data extends JobExecutable {
	...
	protected function calculateTotalSteps() {
		$this->options->totalSteps = 10; // Changed from 9 to 10
	}
	...
	protected function step1() {
		...
		if ( isset( $this->settings->wpSubDirectory ) && '1' === $this->settings->wpSubDirectory ) {
			...
			if ( $this->options->isStagingSite === true ) {
				$newURL = $this->options->existingLive->url; } else {
				$newURL = get_home_url() . '/' . $this->options->cloneDirectoryName; }
	...
	protected function step6() {
		...
		/* update prefix for wp_user_roles in wp_options table */
		$resultOptions = $this->db->query(
			$this->db->prepare(
				"UPDATE {$this->prefix}options SET option_name = replace(option_name, %s, %s) WHERE option_name=%s",
				$this->db->prefix,
				$this->prefix,
				$this->db->prefix . 'user_roles'
			)
		);

		if ( ! $resultOptions ) {
			$this->log( "Search & Replace: Failed to update usermeta meta_key (or option name in options table) database table prefixes; {$this->db->last_error}", Logger::TYPE_ERROR );
			return true;
		}
	...
	/**
	 * Copy specific .htaccess file for uploads/onecom_logs
	 * Only runs when copying Live to Staging
	 * @return bool
	 */
	protected function step10() {
    // Only copy when Live to Staging (not Staging to Live)
    if ( $this->options->isStagingSite === true ) {
        $this->log( "Skipping .htaccess copy (Staging to Live operation)", Logger::TYPE_INFO );
        return true;
    }

    $this->log( "Copying .htaccess for uploads/onecom_logs" );

    // Live to Staging: copy from live to staging
    $sourcePath = ABSPATH . 'wp-content/uploads/onecom_logs/.htaccess';
    $destPath   = ABSPATH . $this->options->cloneDirectoryName . '/wp-content/uploads/onecom_logs/.htaccess';

    // Check if source file exists
    if ( ! file_exists( $sourcePath ) ) {
        $this->log( "Source .htaccess not found at {$sourcePath}, skipping", Logger::TYPE_INFO );
    ...
    if ( ! is_dir( $destDir ) ) {
        if ( ! wp_mkdir_p( $destDir ) ) {
            $this->log( "Failed to create directory {$destDir}", Logger::TYPE_ERROR );
            return false;
        }
    }
    ...
    if ( ! copy( $sourcePath, $destPath ) ) {
        $this->log( "Failed to copy .htaccess from {$sourcePath} to {$destPath}", Logger::TYPE_ERROR );
        return false;
    }

    $this->log( "Successfully copied .htaccess to {$destPath}" );
    return true;
}
	function replaceContentUrls() {
		// replace urls in post content
		$oldUrl = rtrim( $this->parentData->url, '/' );
		$newURL = $oldUrl . '/' . ltrim( $this->options->clone );

		if ( $this->options->isStagingSite === true ) {
			$newURL = rtrim( $this->options->existingLive->url, '/' );
		}

		$this->db->query(
			$this->db->prepare(
				"UPDATE {$this->prefix}posts SET post_content = replace(post_content, '%s', '%s')",
				array( $oldUrl, $newURL )
			)
		);
		// replace urls in post guid
		$this->db->query(
			$this->db->prepare(
				"UPDATE {$this->prefix}posts SET `guid` = replace(`guid`, '%s', '%s')",
				array( $oldUrl, $newURL )
			)
		);
	}

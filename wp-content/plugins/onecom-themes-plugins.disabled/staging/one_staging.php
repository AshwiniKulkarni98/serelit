<?php
/**
 * @package OneStaging
 * @category Core
 * @author one.com
 */
namespace OneStaging;

defined( 'WPINC' ) or die(); // No Direct Access

// Autoloader
require_once __DIR__ . DIRECTORY_SEPARATOR . 'Includes' . DIRECTORY_SEPARATOR . 'Autoloader.php';

use OneStaging\Core\Cache;
use OneStaging\Core\Loader;
use OneStaging\Core\Logger;
use OneStaging\Core\Settings;
use OneStaging\Includes\Autoloader;
use OneStaging\Core\Administrator;

#[\AllowDynamicProperties]
class OneStaging {

	private $services;

	private static $instance;

	private static $settings;

	private function __construct() {
		$file = ONECOM_WP_PATH . 'staging' . DIRECTORY_SEPARATOR . 'one_staging' . '.php';

		$this->slug = plugin_basename( dirname( ( __FILE__ ), 2 ) );

		$this->registerMain();
		$this->registerNamespaces();
		$this->loadWPDB();
		$this->loadDependencies();
		$this->defineHooks();
	}

	/**
	 * Get table prefix of the current site
	 * @return string
	 */
	public static function getTablePrefix() {
		$wpDB = OneStaging::getInstance()->get( 'wpdb' );
		return $wpDB->prefix;
	}

	public static function getContentDir() {
		// check if upload dir is customised using WP option. If so, discard it and use default one.
		$upload_option = get_option( 'upload_path' );
		if ( $upload_option ) {
			update_option( 'upload_path_prev', $upload_option, 'no' );
			delete_option( 'upload_path' );
		}
		$wp_upload_dir = wp_upload_dir();
		$path          = $wp_upload_dir['basedir'] . '/onecom_logs/one_staging';
		wp_mkdir_p( $path );
		return apply_filters( 'wpstg_get_upload_dir', $path . DIRECTORY_SEPARATOR );
	}

	/**
	 * Load WP DB
	 */
	public function loadWPDB() {
		global $wpdb;
		if ( isset( $wpdb ) ) {
			$this->set( 'wpdb', $wpdb );
		}
		return $this;
	}

	/**
	 * Register used namespaces
	 */
	private function registerNamespaces() {
		$autoloader = new Autoloader();
		$this->set( 'autoloader', $autoloader );

		// Autoloader
		$autoloader->registerNamespaces(
			array(
				'OneStaging' => array(
					$this->getSlug() . 'Includes' . DIRECTORY_SEPARATOR,
					$this->getSlug() . 'Includes' . DIRECTORY_SEPARATOR . 'Core' . DIRECTORY_SEPARATOR,
					$this->getSlug() . 'Includes' . DIRECTORY_SEPARATOR . 'Core' . DIRECTORY_SEPARATOR . 'Jobs' . DIRECTORY_SEPARATOR,
				),
			)
		);

		// Register namespaces
		$autoloader->register();
	}


	public function registerMain() {
		// Slug of the plugin
		$this->slug = plugin_basename( dirname( ( __FILE__ ), 2 ) );

		// absolute path to the main plugin dir
		$this->pluginPath = plugin_dir_path( dirname( ( __FILE__ ) ) ) . 'staging' . DIRECTORY_SEPARATOR;

		// absolute URL to the main plugin dir
		$this->pluginURL = plugin_dir_url( __FILE__ );

		// URL to Core folder
		$this->url = plugin_dir_url( __DIR__ ) . 'staging/';
	}


	/**
	 * Define Hooks
	 */
	public function defineHooks() {
		$loader = $this->get( 'loader' );
		$loader->addAction( 'admin_enqueue_scripts', $this, 'enqueueElements', 100 );
		$loader->addAction( 'wp_enqueue_scripts', $this, 'enqueueElements', 100 );
		$loader->addAction( 'admin_bar_menu', $this, 'stagingToolbar', 110 );
	}


	/**
	 * Scripts and Styles
	 * @param string $hook
	 */
	public function enqueueElements( $hook ) {

		$res_ext = ( SCRIPT_DEBUG || SCRIPT_DEBUG == 'true' ) ? '' : '.min';
		$res_dir = ( SCRIPT_DEBUG || SCRIPT_DEBUG == 'true' ) ? '' : 'min-';

		// Load this css file on frontend and backend on all pages if current site is a staging site
		if ( $this->get( 'settings' )->_isStaging === true ) {
			wp_enqueue_style( 'onecom-staging-adminbar', ONECOM_WP_URL . 'assets/css/onecom-admin-staging.css', $this->getVersion() );
		}

		$availablePages = array(
			'onecom-wp-staging',
			'one-com_page_onecom-wp-staging',
		);

		// Load these css and js files only on wp staging admin pages
		if ( ! in_array( $hook, $availablePages ) || ! is_admin() ) {
			return;
		}

		// Load admin js files
		wp_enqueue_script(
			'onecom-stg-admin-script',
			ONECOM_WP_URL . 'assets/' . $res_dir . 'js/onecom_staging' . $res_ext . '.js',
			array( 'jquery' ),
			$this->getVersion(),
			true
		);

		wp_enqueue_script(
			'gravity-icons',
			ONECOM_WP_URL . 'assets/js/index.umd.js',
			[],
			$this->getVersion(),
			true
		);

		wp_localize_script(
			'onecom-stg-admin-script',
			'wpstg',
			array(
				'nonce'        => wp_create_nonce( 'wpstg_ajax_nonce' ),
				'stgID'        => self::get( 'settings' )->getStagingDir(),
				'stgPrefix'    => self::get( 'settings' )->getStagingPrefix(),
				'status'       => __( 'Status', 'Current request status', 'onecom-wp' ),
				'response'     => __( 'Response', 'The message the server responded with', 'onecom-wp' ),
				'copy_msg'     => __( 'Copied successfully.', 'onecom-wp' ),
				'error_msg'    => __( 'Something went wrong; please try again.', 'onecom-wp' ),
				'cpuLoad'      => $this->getCPULoadSetting(),
				'settings'     => (object) array(), // TODO add settings?
				'tblprefix'    => self::getTablePrefix(),
				'msgPrep'      => __( 'Preparing...', 'onecom-wp' ),
				'msgNewStg'    => __( 'Start copying...', 'onecom-wp' ),
				'msgCopyDB'    => __( 'Copying database...', 'onecom-wp' ),
				'msgPrepDirs'  => __( 'Preparing directories...', 'onecom-wp' ),
				'msgCopyFiles' => __( 'Copying files...', 'onecom-wp' ),
				'msgUpdateDB'  => __( 'Updating database...', 'onecom-wp' ),
				'msgFinalize'  => __( 'Finalising...', 'onecom-wp' ),
				'msgFinished'  => __( 'Finished!', 'onecom-wp' ),
				'msgDelStg'    => __( 'Deleting staging...', 'onecom-wp' ),
				'stgDeleted'    => __( 'Your staging site was deleted.', 'onecom-wp' ),
				'stgCreated'    => __( 'Your staging site was created.', 'onecom-wp' ),
				'stgCreatedErr' => __( 'Couldn’t create a staging site. Please try again and contact our support if the issue persists.', 'onecom-wp' ),
				'stgRebuilt'    => __( 'Your staging site was rebuilt.', 'onecom-wp' ),
				'stgRebuiltError'    => __( 'Couldn’t rebuild your staging site. Please try again and contact our support if the issue persists.', 'onecom-wp' ),
				'imageDir'     => ONECOM_WP_URL . 'assets/images',
				'createStagingLabel' => __( 'Create staging site', 'onecom-wp' ),
				'siteUrl'   => trailingslashit( site_url() ),
				'labelView' => __('View your site', 'onecom-wp'),
				'loginStaging' => __('Log in to staging', 'onecom-wp'),
				'scanFail' => __('Failed to scan the files or database required to create the staging environment.', 'onecom-wp'),
				'diskspaceFail' => __('Failed to check the available disk space at the moment.', 'onecom-wp'),
				'deletestagingFail' => __('Failed to delete staging.', 'onecom-wp'),
				'preparestagingtoliveFail' => __('Failed to copy the staging site to the live site.', 'onecom-wp'),
				'prepareclonedataFail' => __('Failed to initiate the staging clone process.', 'onecom-wp'),
				'prepareupdatestagingFail' => __('Failed to update the staging environment.', 'onecom-wp'),
				'cloneDatabaseFail' => __('Failed to copy the database.', 'onecom-wp'),
				'prepareDirectoriesFail' => __('Failed to create directories for the staging site.', 'onecom-wp'),
				'cloneFilesFail' => __('Failed to copy the files for the staging site.', 'onecom-wp'),
				'replaceDataFail' => __('Failed while replacing variables and URLs.', 'onecom-wp'),
				'finishFail' => __('Cloning was mostly successful. The snapshot was copied, but we ran into an issue saving the final details.', 'onecom-wp'),
				'copyStagingToLive' => __('Your staging site was copied to your live site.', 'onecom-wp'),
				'copyStagingToLiveErr' => __('Couldn’t copy your staging site to your live site. Please try again and contact our support if the issue persists.', 'onecom-wp'),

			)
		);
	}

	/**
	 * Staging Toolbar in staging site
	 */
	public function stagingToolbar( $hook ) {
		// exit for network sites
		if ( ! is_network_admin() && is_multisite() ) {
			return false;
		}

		// exit for non-admins
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}

		// exit if live site and no staging exists
		if (
			$this->get( 'settings' )->_isStaging !== true
			&& empty( get_option( 'onecom_staging_existing_staging', array() ) )
		) {
			return;
		}
		$args = array(
			'id'    => 'onecom-staging',
			//'parent' => 'top-secondary',
			'title' => '<span class="ab-icon dashicons dashicons-randomize"></span> <span class="ab-label">' . __( 'Staging', 'onecom-wp' ) . '</span>',
			'href'  => ( is_multisite() && is_network_admin() ) ? network_admin_url( 'admin.php?page=onecom-wp-staging' ) : admin_url( 'admin.php?page=onecom-wp-staging' ),
			'meta'  => array(
				'title' => __( 'Staging', 'onecom-wp' ),
				'class' => 'onecom-wp-stg-toolbar',
			),
		);
		$hook->add_menu( $args );
		return true;
	}

	/**
	 * Get Instance
	 * @return OneStaging
	 */
	public static function getInstance() {
		if ( null === static::$instance ) {
			static::$instance = new static();
		}

		return static::$instance;
	}

	/**
	 * Prevent cloning
	 * @return void
	 */
	private function __clone() {
	}

	/**
	 * Prevent unserialization
	 * @return void
	 */
	public function __wakeup() {
	}

	/**
	 * Load Dependencies
	 */
	private function loadDependencies() {
		// Set loader
		$this->set( 'loader', new Loader() );

		// Set cache
		$this->set( 'cache', new Cache() );

		// Set logger
		$this->set( 'logger', new Logger() );

		// Set settings
		$this->set( 'settings', new Settings() );

		// Set Administrator
		$admin = new Administrator( $this );
	}

	/**
	 * Execute Plugin
	 */
	public function run() {
		$this->get( 'loader' )->run();
	}

	/**
	 * Set a variable to DI with given name
	 * @param string $name
	 * @param mixed $variable
	 * @return $this
	 */
	public function set( $name, $variable ) {
		// It is a function
		if ( is_callable( $variable ) ) {
			$variable = $variable();
		}

		// Add it to services
		$this->services[ $name ] = $variable;

		return $this;
	}

	/**
	 * Get given name index from DI
	 * @param string $name
	 * @return mixed|null
	 */
	public function get( $name ) {
		return ( isset( $this->services[ $name ] ) ) ? $this->services[ $name ] : null;
	}

	/**
	 * @return string
	 */
	public static function getVersion() {
		if ( is_admin() ) :
			$plugin_data = get_plugin_data( __FILE__ );
			return ONECOM_WP_VERSION;
		else :
			$plugin_data = get_file_data( ONECOM_WP_PATH, array( 'Version' => 'Version' ), false );
		endif;
		return ONECOM_WP_VERSION;
	}

	/**
	 * @return string
	 */
	public function getName() {
		$plugin_data = get_plugin_data( ONECOM_WP_PATH );
		return $plugin_data['Name'];
	}

	/**
	 * @return string
	 */
	public static function getSlug() {
		return __DIR__ . DIRECTORY_SEPARATOR;
	}

	/**
	 * Get path to main plugin file
	 * @return string
	 */
	public function getPath() {
		return dirname( __DIR__, 1 );
	}

	/**
	 * Get main plugin url
	 * @return string
	 */
	public function getUrl() {
		return plugin_dir_url( __DIR__ );
	}

	/**
	 * @return array|mixed|object
	 */
	public function getCPULoadSetting() {
		$options = $this->get( 'settings' );
		$setting = $options->getCpuLoad();

		switch ( $setting ) {
			case 'high':
				$cpuLoad = 0;
				break;

			case 'medium':
				$cpuLoad = 1000;
				break;

			case 'low':
				$cpuLoad = 3000;
				break;

			case 'default':
			default:
				$cpuLoad = 1000;
		}

		return $cpuLoad;
	}
}
